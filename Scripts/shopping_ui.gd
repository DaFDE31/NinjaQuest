...
@onready var buying_container: GridContainer = %BuyingContainer
@onready var selling_container: GridContainer = %SellingContainer
@onready var buy_button: Button = %BuyButton
@onready var sell_button: Button = %SellButton

func setup_buying_grid():
	for child in buying_container.get_children():
		child.queue_free()
	for i in items_to_buy.size():
		...
		buying_container.add_child(buying_slot)
...
func on_buy_slot_clicked(idx: int):
	if selected_buy_item_indexes.has(idx):
		buying_container.get_child(idx).toggle_button_selected_variation(false)
		selected_buy_item_indexes.erase(idx)
	else:
		buying_container.get_child(idx).toggle_button_selected_variation(true)
...
func on_selling_slot_clicked(idx: int):
	if selected_sell_item_indexes.has(idx):
		selling_container.get_child(idx).toggle_button_selected_variation(false)
		selected_sell_item_indexes.erase(idx)
	else:
		selling_container.get_child(idx).toggle_button_selected_variation(true)
