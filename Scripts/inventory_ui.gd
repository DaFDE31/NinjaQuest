...
signal drop_item_on_ground(idx, int)
...
func _ready():
	...
	for i in size:
		...
		inventory_slot.drop_item.connect(func(): drop_item_on_ground.emit(i))
...
func clear_slot_at_index(idx: int):
	var empty_inventory_slot: InventorySlot = INVENTORY_SLOT.instantiate()
	toggle()
	empty_inventory_slot.drop_item.connect(func(): drop_item_on_ground.emit(idx))
...
func set_selected_spell_index(idx: int):
	for i in spell_slots.size():
		spell_slots[i].toggle_button_selected_variation(idx == i)

func toggele_spells_ui(is_visible: bool):
